...
from utils import get_output, mkdir_if_missing
...
def get_single_task_meter(p, database, task):
    ...
    if task == 'semseg':
        from mtl_evaluation.eval_semseg import SemsegMeter
        return SemsegMeter(database)

    elif task == 'human_parts':
        from mtl_evaluation.eval_human_parts import HumanPartsMeter
        return HumanPartsMeter(database)

    elif task == 'normals':
        from mtl_evaluation.eval_normals import NormalsMeter
        return NormalsMeter()

    elif task == 'sal':
        from mtl_evaluation.eval_sal import SaliencyMeter
        return SaliencyMeter()

    elif task == 'depth':
        from mtl_evaluation.eval_depth import DepthMeter
        return DepthMeter()

    elif task == 'edge': # Single task performance meter uses the loss (True evaluation is based on seism evaluation)
        from mtl_evaluation.eval_edge import EdgeMeter
...
def validate_results(p, current, reference, tasks=None):
    """
    ...
    """
    tasks = p.TASKS.NAMES if tasks is None else tasks
...
@torch.no_grad()
def save_model_predictions(mtl_configs, val_loader, model, save_dir_root, client_idx, tasks=None):
    """ Save model predictions for all tasks """

    # print('Save model predictions to {}'.format(save_dir_root))
    model.eval()
    tasks = p.TASKS.NAMES if tasks is None else tasks
    save_dirs = {
        task: os.path.join(save_dir_root, f'client_{client_idx}', task)
        for task in tasks
    }
    ...
    for ii, sample in enumerate(val_loader):
        ...
        for task in tasks:
            ...
            for jj in range(int(inputs.size()[0])):
                if (len(sample[task][jj].unique()) == 1 and
                    sample[task][jj].unique() == 255):
                    continue
                fname = meta['image'][jj]
                result = cv2.resize(output_task[jj],
                                    dsize=(meta['im_size'][1][jj],
                                           meta['im_size'][0][jj]),
                                    interpolation=mtl_configs[task]['task_infer_flagval'])
                if task == 'depth':
                    sio.savemat(os.path.join(save_dirs[task], fname + '.mat'),
                                {'depth': result})
                else:
                    imageio.imwrite(os.path.join(save_dirs[task], fname + '.png'),
                                    result.astype(np.uint8))


def eval_all_results(save_dir_root, database, client_idx, tasks, dataidx=None, overfit=False):
    """Evaluate results for every task by reading the predictions from the save dir """
    save_dir = os.path.join(save_dir_root, f'client_{client_idx}')
    results = {}

    for task in tasks:
        if task == 'edge':
            # raise NotImplementedError('matlab is not ready for edge evaluation')
            from .eval_edge import eval_edge_predictions
            results['edge'] = eval_edge_predictions(
                p, database=database, save_dir=save_dir,
                dataidx=dataidx)

        elif task == 'semseg':
            from .eval_semseg import eval_semseg_predictions
            results['semseg'] = eval_semseg_predictions(
                database=database, save_dir=save_dir, overfit=overfit,
                dataidx=dataidx)

        elif task == 'human_parts':
            from .eval_human_parts import eval_human_parts_predictions
            results['human_parts'] = eval_human_parts_predictions(
                database=database, save_dir=save_dir, overfit=overfit,
                dataidx=dataidx)

        elif task == 'normals':
            from .eval_normals import eval_normals_predictions
            results['normals'] = eval_normals_predictions(
                database=database, save_dir=save_dir, overfit=overfit,
                dataidx=dataidx)

        elif task == 'sal':
            from .eval_sal import eval_sal_predictions
            results['sal'] = eval_sal_predictions(
                database=database, save_dir=save_dir, overfit=overfit,
                dataidx=dataidx)

        elif task == 'depth':
            from .eval_depth import eval_depth_predictions
            results['depth'] = eval_depth_predictions(
                database=database, save_dir=save_dir, overfit=overfit,
                dataidxs=dataidx)

        elif 'celeba' in task:
            raise NotImplementedError('Evaluation for CelebA multi-task '
                                      'binary classification is not'
                                      'implemented yet.')
