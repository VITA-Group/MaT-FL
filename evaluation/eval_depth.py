...
def eval_depth_predictions(database, save_dir, overfit=False, dataidxs=None):

    # Dataloaders
    if database.lower() == 'nyud':
        from datasets.nyud import NYUD_MT_truncated
        gt_set = 'val'
        db = NYUD_MT_truncated(split=gt_set, tasks=['depth'], overfit=overfit, dataidxs=dataidxs)
