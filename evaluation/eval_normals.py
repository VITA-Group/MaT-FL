...
def eval_normals_predictions(database, save_dir, overfit=False, dataidx=None):
    """ Evaluate the normals maps that are stored in the save dir """

    # Dataloaders
    if database.lower() == 'pascalcontext':
        from datasets.pascal_context import PASCALContext
        gt_set = 'val'
        db = PASCALContext(split=gt_set, tasks=['normals'], overfit=overfit, dataidxs=dataidx)
    elif database.lower() == 'nyud':
        from datasets.nyud import NYUD_MT_truncated
        gt_set = 'val'
        db = NYUD_MT_truncated(split=gt_set, tasks=['normals'], overfit=overfit, dataidxs=dataidx)
